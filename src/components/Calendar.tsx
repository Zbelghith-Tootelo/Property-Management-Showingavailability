  ...
  return (
    <div className="flex bg-white rounded-[16px] border border-[#e5e7eb] shrink-0 mx-[32px] max-[1000px]:mx-[12px] overflow-hidden" style={{height:56}}>
      ...
      {weekDays.map((d,i)=>{
        ...
        return (
          <div key={i} className={`flex-1 h-full flex flex-col items-center justify-center gap-[2px] border-l border-[#f0f0f0] ${isToday ? "bg-[#213163]" : ""}`}>
            <span className={`font-semibold text-[13px] ${isToday ? "text-white" : "text-[#9ca3af]"}`}>{DAY_NAMES[i]}</span>
            <span className={`font-bold text-[16px] ${isToday ? "text-white" : "text-[#1b2559]"}`}>{d.getDate()}</span>
          </div>
        );
      })}
    </div>
